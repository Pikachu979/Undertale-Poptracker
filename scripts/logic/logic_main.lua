...
undertale_location = {}
undertale_location.__index = undertale_location
...
function undertale_location.new(name)
    local self = setmetatable({}, undertale_location)
    if name then
...
function undertale_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        exit = undertale_location.new(exit)
    end
    if rule == nil then
        rule = always
    end
...
function undertale_location:connect_two_ways(exit, rule)
...
function undertale_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function undertale_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function undertale_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function undertale_location:accessibility()
...
-- 
function undertale_location:discover(accessibility, keys)
...
entry_point = undertale_location.new("entry_point")
